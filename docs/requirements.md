...
| **RNF-FE08** | Variables de entorno gestionadas vía `.env.local` (no commiteado) y `.env.example` (commiteado, con placeholders). Variables prefijadas con `VITE_*`.                     | `.gitignore` incluye `.env.local`; arranque sin env falla con mensaje claro.         | Vite best practices        |
